...
replace_block('function WeekPage(', 'function ModelConsensus(', week_page, 'WeekPage')
...
replace_block('function ModelConsensus(', 'function QuickNumbers(', model_consensus, 'ModelConsensus')
...
replace_block('function MatchupMeter(', 'function ForecastMovement(', matchup_meter, 'MatchupMeter')

movement_component = r'''function MovementAttribution({ movement }) {
...
insert_at = text.find('function GameModal(')
if insert_at < 0:
    raise SystemExit('GameModal marker not found')
if 'function MovementAttribution(' not in text:
    text = text[:insert_at] + movement_component + '\n\n' + text[insert_at:]
...
text = text.replace('<div className="pub-case-grid vnext-case-grid"><div><span>CASE FOR {game.pick}</span>','<div className="pub-case-grid vnext-case-grid"><div><span>HOW {game.pick} WINS</span>')
text = text.replace('<div><span>CASE FOR {opponent}</span>','<div><span>HOW {opponent} WINS</span>')
...
replace_block('function TeamsPage(', 'function RatingsPage(', teams_block, 'Teams/TeamModal')
...
replace_block('function RatingsPage(', 'function HistoryPage(', ratings, 'RatingsPage')
...
replace_block('function HistoryPage(', 'function MethodPage(', history, 'HistoryPage')
...
print('Applied front-facing Sunday Signal polish.')
