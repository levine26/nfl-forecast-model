...
test('History remains a first-class 2026 receipts surface with calibration and signal-tier accountability', async ({ page }) => {
  ...
  await expect(page.getByText('PERFORMANCE HISTORY', { exact: true })).toBeVisible()
  await expect(page.getByRole('heading', { name: /Every official LevLine bet, tracked/i })).toBeVisible()
  await expect(page.getByText('THIS WEEK', { exact: true })).toBeVisible()
  await expect(page.getByText(/WEEK \d+ TOTALS/)).toBeVisible()
