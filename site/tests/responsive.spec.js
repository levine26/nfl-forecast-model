...
test('History remains a first-class 2026 receipts surface with calibration and signal-tier accountability', async ({ page }) => {
  ...
  await expect(page.getByText('LEVLINE TRACK RECORD', { exact: true })).toBeVisible()
  await expect(page.getByRole('heading', { name: /Official 2026 betting record/i })).toBeVisible()
  await expect(page.getByText(/THIS WEEK · W\d+/)).toBeVisible()
  await expect(page.getByText(/WEEK \d+ SUMMARY/)).toBeVisible()
  await expect(page.getByText('SEASON PERFORMANCE', { exact: true })).toBeVisible()
  await expect(page.getByText(/What 1 unit on every LevLine pick would have done/i)).toHaveCount(0)
  await expect(page.getByText('BET TRACKER', { exact: true })).toHaveCount(0)
