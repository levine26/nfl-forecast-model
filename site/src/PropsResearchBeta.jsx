...
function LineCard({forecast}) {
  ...
  return <article className={`lp-card ${forecast.signal_state==='NO SIGNAL'?'no-signal':''}`}>
    ...
    <div className="lp-grid metrics-primary"><Metric label="LevLine Over" value={formatPercent(forecast.model?.over_probability)} emphasis/><Metric label="LevLine Under" value={formatPercent(forecast.model?.under_probability)}/><Metric label="Market No-Vig" value={formatPercent(forecast.model?.market_no_vig_probability)}/><Metric label="Probability Edge" value={formatPoints(forecast.model?.probability_edge)} emphasis/></div>
