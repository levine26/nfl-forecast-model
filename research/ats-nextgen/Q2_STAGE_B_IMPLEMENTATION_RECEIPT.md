...
**Pre-result implementation head:** `a857d1864a9f94da80fb61d1e7385c4eabcddf88`

## 1. Pre-result evidence boundary

This receipt is written before accepting any Q2 historical result. A prior contract-only Q2 workflow succeeded on an earlier pre-result head; the current implementation head has started contract CI but its historical job has not begun at the time this receipt is frozen.
...
Any Q2 workflow run from a head preceding this receipt is superseded for evidentiary purposes. Only an exact-head run that includes this receipt and passes the full contract may be accepted as Stage-B evidence.
...
The scientific implementation frozen at pre-result head `a857d1864a9f94da80fb61d1e7385c4eabcddf88` is:
...
- opening machine registry `research/ats-nextgen/phase2_q2_opening_registry.json`: blob `81e2ed4a663b43248edc89dd8d9d5b4ee548e9ac`.
...
The historical experiment is limited to the bounded Stage-B surface already specified in `Q2_STAGE_B_OPENING_RECEIPT.md`:
...
- prior-only pooled discrete CRPS for inner selection;
