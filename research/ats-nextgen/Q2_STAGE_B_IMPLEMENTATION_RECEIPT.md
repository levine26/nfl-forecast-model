...
**Status at receipt:** **IMPLEMENTATION AND REPORTING FROZEN; NO ACCEPTED Q2 HISTORICAL RESULT**  
**Branch:** `research/ats-nextgen-phase2-q2`  
**Stage-A merge / Stage-B base:** `a2581a62e3797a6ac466d614326bc72b7d5a1c57`  
**Canonical reporting-complete pre-result head:** `71aeb5734ceaf2b6c384f88f68314442b205272e`

## 1. Pre-result evidence boundary

This receipt is finalized before accepting any Q2 historical result. Earlier Stage-B heads and workflow attempts are superseded for evidentiary purposes.

A temporary pre-result development state contained an unused alternative `q2_eval.py` / `q2_runtime.py` implementation path. It was identified as unnecessary before any accepted Q2 evidence and removed without changing the scientific contract, candidate grids, fitting rules, or canonical runner. The canonical implementation therefore has one execution path: `challenger_ats_nextgen_q2.py` + `challenger_ats_nextgen_q2_experiment.py` + `challenger_ats_nextgen_q2_reporting.py` + the fail-closed runner.

The reporting-complete head also closes preregistered reporting gaps before results by freezing: direct final-margin key-mass calibration, cover calibration intercept/slope, and equal-tailed central interval coverage at exactly 50%, 80%, and 90%. These additions are diagnostic/reporting only and do not modify Q2 fitting, tuning, arm identity, or the primary comparison.
...
Only an exact-head run that includes the finalized receipts/registry, passes the Q2 contract, and then completes the historical job may be accepted as Stage-B evidence.
...
The canonical scientific implementation at pre-result head `71aeb5734ceaf2b6c384f88f68314442b205272e` is:
...
- Q2 reporting contract `src/nfl_forecast/challenger_ats_nextgen_q2_reporting.py`: blob `596b9ede2c5c1f15bcac440d07dd8cb80e03339e`;
- fail-closed Q2 historical runner `scripts/run_challenger_ats_nextgen_q2.py`: blob `af73910ce78c51e18b879d6175a8873f11bae40e`;
- Q2 core tests `tests/test_challenger_ats_nextgen_q2.py`: blob `4a691a354b1e577fb64ae66767bee6eb2e985fc2`;
- Q2 experiment/reporting tests `tests/test_challenger_ats_nextgen_q2_experiment.py`: blob `339fc5d8d315767d860f4d079a9c43a0304ba6d0`;
- Q2 Stage-B workflow `.github/workflows/research_ats_nextgen_q2.yml`: blob `1c5c000ada73cfe458c69a1e440bafbbb1a2ca75`;
...
- scoring/reporting receipt `research/ats-nextgen/Q2_STAGE_B_SCORING_RECEIPT.md`: blob `23a595ed7bbab8ff7eabd59225bc533d2230b85f`.

The runner blob above checks the gate hash, full Q1 OOF hash, and direct per-season Q1 median identity before the first Q2 fit.

## 4. Frozen experiment and reporting surface
...
- cover reliability bins exactly `[0,.1),...,[.9,1]`;
- cover calibration intercept/slope via unpenalized logistic recalibration fit for reporting only;
- equal-tailed central interval diagnostics exactly 50%, 80%, 90%;
- direct predicted-vs-observed `P(|M|=a)` calibration for `a={3,6,7,10,14}` overall and by season;
- no ATS hit rate or ROI used for model selection.

The primary incremental comparison remains `Q2_GN_FULL` versus paired `M1_GN_FULL` on exact common OOF rows. Reference/ablation/diagnostic arms may explain mechanism but may not redefine the primary candidate after results.
