...
This module is research-only. It consumes the preserved strict-v2 source outputs and
asks a narrower source question: among official rows that actually carry a recognized
practice participation state, does reverse identity resolve at the frozen rate while
all existing practice-state, chronology, schedule, and source-integrity gates hold?

It does not fit P(active), use game outcomes, inspect completed-2026 outcomes, or alter
production.
"""
...
def _recognized_mask(values: pd.Series) -> pd.Series:
    normalized = values.map(normalize_practice_status)
    return normalized.isin(RECOGNIZED)
...
def practice_state_reverse_identity_metrics(
    ...
    unresolved_recognized = (
        int(_recognized_mask(unresolved["external_practice_status"]).sum())
        if len(unresolved)
        else 0
    )
    ...
    return {
        ...
        "official_practice_state_to_canonical_identity_resolution_rate": (
            resolved_recognized / official_recognized
        ),
...
def evaluate(
    *,
    ...
    summary_by_season = {
        int(row["season"]): row
        for _, row in season_summaries.iterrows()
    }
    for season in required:
        ...
        if row is None:
            ...
            continue
        official_path = v2_output_dir / f"official_nfl_regular_crosscheck_{season}.csv"
        unresolved_path = v2_output_dir / f"official_identity_unresolved_{season}.csv"
