...
This module is research-only. It applies the coefficients frozen in CHALLENGER_GRID.json
to already-frozen Props 2.1 prospective rows. It does not read outcomes, refit weights,
modify LevLine/F-ST, or publish production artifacts.
"""
...
def apply_challenger(
    ...
    *,
    contract_version: str = 'levline-props-2.2-prereg-v0.1',
) -> dict[str, Any]:
    ...
    market_weight = _num(challenger.get('line_market_weight'))
    model_weight = _num(challenger.get('line_model_weight'))
    shrink = _num(challenger.get('probability_shrink_to_half'))
    if market_weight is None or model_weight is None or shrink is None:
        raise Props22Error(f'{challenger_id}: challenger coefficients must be numeric')
    if not (0.0 <= market_weight <= 1.0 and 0.0 <= model_weight <= 1.0):
        raise Props22Error(f'{challenger_id}: line weights must be within [0, 1]')
    if abs((market_weight + model_weight) - 1.0) > 1e-12:
        raise Props22Error(f'{challenger_id}: line weights must sum to one')
    if not 0.0 <= shrink <= 1.0:
        raise Props22Error(f'{challenger_id}: probability shrinkage must be within [0, 1]')
    ...
    elif market_weight > 0.0 and market_line is None:
        line_unavailable_reason = 'missing_market_line_for_residual_challenger'
    else:
        anchor = market_line if market_weight > 0.0 else 0.0
        challenger_line = model_weight * model_line + market_weight * anchor
        line_available = True

    probability_available = model_probability is not None
    ...
    if model_probability is None:
        ...
        challenger_probability = 0.5 * shrink + model_probability * (1.0 - shrink)
