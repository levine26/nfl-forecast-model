...
def record_shadow_a(
    ...
    *,
    ...
    if not source_workflow_run:
        raise FootballShadowError("source_workflow_run is required")
    for label,value in (
        ...
        if len(value) not in {40,64} or any(ch not in "0123456789abcdef" for ch in value):
            raise FootballShadowError(f"{label} must be a git SHA")
    if not source_market_provider:
        raise FootballShadowError("source_market_provider is required")
    if not source_market_credential_mode:
        raise FootballShadowError("source_market_credential_mode is required")
    if len(source_provenance_sha256)!=64 or any(
        ch not in "0123456789abcdef" for ch in source_provenance_sha256
    ):
        raise FootballShadowError("source_provenance_sha256 must be SHA-256")
