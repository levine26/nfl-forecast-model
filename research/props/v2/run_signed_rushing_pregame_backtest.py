...
"""Paired historical pregame ablation for signed rushing-yard event support."""
...
def interval_score(samples: np.ndarray, observation: float, level: float=0.80):
...
def cluster_ci(frame: pd.DataFrame, column: str, *, seed: int, replicates: int=3000):
...
def run_season(*, season: int, simulations: int, require_genuine_open: bool=True):
    history_start=2021
    ...
    roster=roster_metadata(market,book_id=PRIMARY_BOOK,require_genuine_open=require_genuine_open)
    event_map,event_map_audit=map_events_to_schedule(game_lines,schedule)
    observations,market_pair_audit=market_observations(
        market,book_id=PRIMARY_BOOK,require_genuine_open=require_genuine_open
    )
    observations=observations[
        observations["event_id"].isin(event_map)
        & observations["prop_type"].astype(str).eq("rushing_yards")
    ].copy()

    actual_stats,participation,participation_audit=build_game_stats_and_participation(pbp,snap_counts)
    ...
    rush_events=build_rushing_event_rows(pbp,player_positions(players))
    pools,pool_audit=fit_residual_pools(rush_events,trained_through_season=trained_through)
    ...
    for week in range(1,19):
        ...
        for event_id in events:
            ...
            state,qb_overrides,state_audit=build_market_listed_player_state(event_rows,mapped)
            if state is None:
            ...
            residual=residual_efficiency_by_team_from_empirical_priors(game_teams,fitted_priors)
            ...
            try:
                ...
                overlay_audit=apply_signed_rushing_overlay(
                    challenger,game_input,pools,seed_namespace=game_id
                )
                if overlay_audit["carry_arrays_modified"]!=0:
                    raise RuntimeError("signed rushing overlay modified carry samples")
            ...
            for _,obs in obs_by_event[event_id].iterrows():
                ...
                actual=float(actual_stats.get((game_id,player_id),{}).get("rushing_yards",0.0))
                b_samples=baseline.player_stats[player_id]["rushing_yards"]
                c_samples=challenger.player_stats[player_id]["rushing_yards"]
                ...
                b_is,b_cov,b_width=interval_score(b_samples,actual)
                c_is,c_cov,c_width=interval_score(c_samples,actual)
                ...
                results.append({
                    ...
                    "v1_interval_width_80":b_width,
                    "challenger_interval_width_80":c_width,
                    "v1_grading_result":grade(b_side),
                    "challenger_grading_result":grade(c_side),
                    "carry_samples_identical":bool(np.array_equal(
                        baseline.player_stats[player_id]["carries"],
                        challenger.player_stats[player_id]["carries"],
                    )),
                })

    frame=pd.DataFrame(results)
    if frame.empty:
        raise RuntimeError("no paired signed-rushing rows")
    ...
    summary={
        ...
        "crps_difference_ci95":cluster_ci(
            frame,"challenger_minus_v1_crps",seed=20260918+int(season)
        ),
        ...
        "mae_difference_ci95":cluster_ci(
            frame,"challenger_minus_v1_abs_error",seed=20261918+int(season)
        ),
        ...
        "all_carry_samples_identical":bool(frame["carry_samples_identical"].all()),
        "residual_pool_audit":pool_audit,
        "source_audit":{
...
def main()->int:
    ...
    parser.add_argument("--season",type=int,required=True,choices=[2023,2024,2025])
    parser.add_argument("--simulations",type=int,default=5000)
    parser.add_argument("--output-dir",type=Path,required=True)
    args=parser.parse_args()

    frame,summary=run_season(season=args.season,simulations=args.simulations)
    args.output_dir.mkdir(parents=True,exist_ok=True)
    frame.to_csv(args.output_dir/f"{args.season}_forecast_level.csv",index=False)
