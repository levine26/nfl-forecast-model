...
"""Props 2.0 retrospective latent dynamic-role V2 challenger backtest.

This is research-development evidence, not promotion evidence. The V1 forecast model
remains frozen. The only challenger input added here is strictly lagged offensive snap
share, used as a player-role proxy through the existing provenance-controlled role
multipliers. Target-game snap counts remain grading-only and never enter the forecast.
"""
    ...
    *,
    ...
    for week in range(int(week_start), int(week_end) + 1):
        ...
        for event_id in week_obs_events:
            ...
            role_adjustments_by_team = {}
            role_audit_by_team = {}
            for role_team in teams:
                adjustments, role_audit = build_dynamic_role_v2_adjustments(
                    snap_counts,
                    state,
                    season=season,
                    week=week,
            ...
            try:
                ...
                package = build_game_upstream_package(
                    ...
                    role_adjustments_provenance=(
                        f"{DYNAMIC_ROLE_VERSION}:{role_mode}:strictly_lagged_latent_snap_state"
                    ),
                )
    ...
    summary = {
        ...
        "challenger": "dynamic_role_latent_state",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Props 2.0 dynamic-role retrospective challenger backtest.")
