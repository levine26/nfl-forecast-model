...
def test_all_preregistered_defective_weeks_have_matchup_evidence() -> None:
    ...
    assert all(CONTRACT["registered_matchup_evidence"][f"{season}-{week}"] for season, week in defects)


def test_fallback_qualifies_only_timestamp_safe_row(monkeypatch) -> None:
    ...
    v3._FALLBACK_EVIDENCE[(2023, 20)] = {
        "source_presence_gate_passed": True,
        "sources": [{"url": "https://example.test/report"}],
    }
...
def test_fallback_late_row_remains_in_fallback_denominator_and_unqualified(monkeypatch) -> None:
    monkeypatch.setattr(
        v3,
        "_ORIGINAL_BUILD",
        lambda *args, **kwargs: _canonical_row(timestamp="2024-01-20T22:00:00Z"),
    )
    monkeypatch.setattr(v3, "_DEFECTIVE", {(2023, 20)})
    v3._FALLBACK_EVIDENCE.clear()
    v3._FALLBACK_EVIDENCE[(2023, 20)] = {
