...
from research.levline4_2026_ngs_player_id_browser_heldout_v4 import (
    evaluate_gates,
    name_key,
    query_plan,
    resolve_target,
...
def test_contract_freezes_new_out_of_sample_population_and_governance():
    c = load_contract()
    assert c["contract_id"] == "LEVLINE-4-2026-NGS-PLAYER-ID-BROWSER-HELDOUT-V4"
    assert c["preregistration_state"]["v3_target_rows_may_not_be_used_for_v4_validation"] is True
...
def test_name_key_and_query_cascade_are_narrow_and_deterministic():
...
def test_team_equivalence_is_only_ari_az():
...
def test_position_and_jersey_tiebreak_resolves_only_unique_joint_match():
    ...
    r = resolve_target(target, payload)
    assert r["primary_candidate_count"] == 2
    assert r["tiebreak_applied"] is True
    assert r["tiebreak_succeeded"] is True
    assert r["selected_gsis_id"] == "00-0040094"
    ...
    r = resolve_target(mismatch, payload)
    assert r["tiebreak_applied"] is True
    assert r["tiebreak_succeeded"] is False
    assert r["resolved"] is False
    assert r["selected_gsis_id"] is None
...
def _synthetic_success():
    ...
    for i in range(64):
        ...
        results.append({
            ...
            "selected_candidate": {"gsisId": f"00-{i:07d}"},
    ...
    for i in range(2, 4):
        ...
        results.append({
            ...
            "selected_candidate": {"gsisId": f"00-{100+i:07d}"},
            "selected_candidate_gsis_valid": True,
    ...
    for i in range(8, 16):
        ...
        targets.append(target)
        attempts = [{"http_status": 200, "parseable_players_array": True, "player_count": 0}, {"http_status": 200, "parseable_players_array": True, "player_count": 1}] if i == 8 else [{"http_status": 200, "parseable_players_array": True, "player_count": 1}]
        results.append({
            "target_row_identity": [f"V{i}", f"Variant {i}", f"/v{i}"],
            "resolved": True,
            "selected_gsis_id": f"00-{200+i:07d}",
            "selected_candidate": {"gsisId": f"00-{200+i:07d}"},
            "selected_candidate_gsis_valid": True,
            "jersey_comparable": False,
            "jersey_agrees": None,
            "query_attempts": attempts,
            "tiebreak_applied": i == 9,
            "tiebreak_succeeded": i == 9,
        })
    return targets, results


def test_gate_logic_and_conditional_capability_flags():
...
def test_authority_ceiling_remains_research_only_even_on_pass():
    a = load_contract()["authority_if_and_only_if_all_v4_gates_pass"]
    assert a["ngs_repaired_name_team_resolver_qualified_for_frozen_v4_population"] is True
    assert a["official_roster_to_ngs_gsis_candidate_bridge_qualified_for_separately_preregistered_heldout_application"] is True
    for key in (
