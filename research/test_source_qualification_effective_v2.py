...
def test_rights_metadata_is_retained_but_removed_from_technical_failure_axis() -> None:
    ...
    assert any("rights boundary" in note.lower() for note in row["rights_governance_notes"])
    assert all("rights boundary" not in failure.lower() for failure in row["technical_known_source_failures"])
    assert any(
        "current endpoint cannot reconstruct past state" in failure.lower()
