...
def test_candidate_index_and_audit_preserve_unknown_and_ambiguity_states():
    ...
    frame = pl.DataFrame(records).select(list(SOURCE_FIELDS))
...
def test_full_validation_refuses_nonfrozen_projection_before_network(tmp_path: Path):
    fake_projection = tmp_path / "projection.parquet"
    records = [
        {
            "season": 2026,
            "game_type": "REG",
    ...
    pl.DataFrame(records).select(list(SOURCE_FIELDS)).write_parquet(fake_projection)
    ...
    def fake_get(url, **kwargs):
        nonlocal called
        called = True
        raise AssertionError("network should not be reached for a bad projection SHA")

    with pytest.raises(RuntimeError, match="identity projection sha256 mismatch"):
        run_validation(
            fake_projection,
            tmp_path / "out",
            get=fake_get,
