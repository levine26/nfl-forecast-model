...
_SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "run_challenger_v08.py"
_SPEC = importlib.util.spec_from_file_location("run_challenger_v08_test_module", _SCRIPT)
assert _SPEC is not None and _SPEC.loader is not None
_RUNNER = importlib.util.module_from_spec(_SPEC)
_SPEC.loader.exec_module(_RUNNER)
build_candidate_shadow_slate = _RUNNER.build_candidate_shadow_slate
...
def test_full_shadow_slate_precommits_distinct_replayable_candidates_per_game():
    ...
    research = pd.DataFrame({
        "season": np.repeat([2022, 2023, 2024, 2025], 30),
        "home_win": y,
        "pure_prob": np.where(y == 1.0, 0.62, 0.38),
        "market_prob": np.where(y == 1.0, 0.59, 0.41),
    })
    adaptive = {
        "Hybrid adaptive Brier": {
            "feature_set": "production_compatible",
            "kind": "linear",
            "objective": "brier",
        },
        "Logit hybrid adaptive Brier": {
            "feature_set": "production_compatible",
            "kind": "logit",
            "objective": "brier",
        },
    ...
    slate = build_candidate_shadow_slate(
        current=current,
        current_pure_by_set={"production_compatible": np.array([0.61, 0.44])},
        research_by_set={"production_compatible": research},
        feature_sets={"production_compatible": []},
        adaptive_candidates=adaptive,
        selected_label="Hybrid adaptive Brier",
    )

    assert len(slate) == 8
    assert slate.research_candidate.nunique() == 4
    assert not slate.duplicated(
        ["game_id", "challenger_version", "research_candidate"]
    ).any()
    assert slate.groupby("game_id").selected_shadow_candidate.sum().eq(1).all()
    assert slate[slate.selected_shadow_candidate].research_candidate.eq(
        "Hybrid adaptive Brier"
    ).all()
    assert set(slate.research_method) == {"pure", "fixed", "linear", "logit"}
    assert slate.effective_pure_weight.min() >= 0.25
    assert slate.challenger_version.eq("0.8-qb-starter").all()
