...
"""Fail-closed direct-source backfill for Copilot-written Sunday Signal Reads.

The writer is expected to research each matchup, but a slate response can still return
fewer than two direct report URLs. This module may fill only that provenance gap by
querying Bing News RSS against approved publishers. A candidate is accepted only if
the centralized source policy classifies the resolved target as a direct report.
It never touches LevLine inputs or numerical outputs.
"""
...
def canonical_direct_url(url: str) -> str:
    """Return a direct approved report URL, including a resolvable Bing target."""
    ...
    *,
    ...
    try:
        response = session.get(
            url,
            timeout=timeout,
            headers={"User-Agent": "Sunday-Signal/1.0 (+public NFL research)"},
        )
    ...
    requested_family = media_domain_family(f"https://{domain}/")
    for row in rows:
        ...
        if not direct or media_domain_family(direct) != requested_family:
            continue
...
def backfill_direct_sources(
    ...
    *,
    ...
    """Add direct report URLs until the independent-source gate is met.

    Existing rows that do not pass the centralized direct-report policy are discarded.
    If two independent valid reports cannot be established, the caller still receives
    fewer than ``minimum_sources`` and the publication validator fails closed.
    """
    ...
    selected = [
        ...
        if isinstance(source, dict) and is_direct_media_report_url(str(source.get("url") or ""))
    ]
    families = {
        media_domain_family(str(source.get("url") or ""))
        for source in selected
        if media_domain_family(str(source.get("url") or ""))
    }
    titles = {
        re.sub(r"[^a-z0-9]+", " ", str(source.get("title") or "").lower()).strip()
        for source in selected
    }
    ...
    for domain in domains:
        if len(families) >= minimum_sources or len(selected) >= maximum_sources:
            break
        if media_domain_family(f"https://{domain}/") in families:
            continue
        for candidate in _search_domain(
            ...
            family = media_domain_family(candidate["url"])
