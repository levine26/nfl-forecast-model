...
def _history_paragraph(history: list[dict[str, Any]], coaching: list[dict[str, Any]]) -> str | None:
    if history and coaching:
        ...
        return f"History matters here, but only after adjusting for what changed. {hist} What is different now: {change} That is why Sunday Signal treats the old result as evidence, not destiny."
    if history:
