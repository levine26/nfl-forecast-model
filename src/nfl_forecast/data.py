...
def _load_pbp_with_live_fallback(seasons: list[int]):
    """Load PBP while tolerating only a not-yet-published newest-season asset.

    nflreadpy has represented a lagging live-season PBP feed in two ways: an
    up-front ValueError guard and, with newer downloader behavior, a 404 for the
    newest play_by_play_<season>.parquet release. Both are safe to handle by
    preserving the live schedule while using the latest published historical PBP.
    Any other download failure or any missing historical season remains fatal.
    """
    try:
...
def load_core_data(seasons: Iterable[int], cache_dir: str = ".cache/nflreadpy") -> NFLDataBundle:
    ...
    # The nflverse schedule is the authoritative live scaffold even when PBP publication lags.
    ...
    # Keep the live schedule/results/Elo layer while EPA/form remains through the
    # latest actually published PBP season. The fallback is deliberately fail-closed.
