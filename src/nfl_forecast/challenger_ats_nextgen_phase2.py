...
def build_historical_ats_gate(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    push = eligible & np.isclose(out["ats_residual"].to_numpy(dtype=float), 0.0, atol=1e-9, rtol=0.0)
...
def validate_gate_frame(frame: pd.DataFrame) -> None:
    ...
    expected_outcome = np.where(push, "PUSH", np.where(actual.to_numpy() > 0, "HOME_COVER", "HOME_LOSS"))
