...
def build_chronological_logit_stack(
    ...
    *,
    ...
    Required inputs are themselves historical OOF probabilities: ``market_prob`` and
    ``pure_prob``. For target season S, the meta-model sees only rows with season < S.
    No target-season outcome can affect the target-season stack prediction.
    """
